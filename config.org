...
* Web Development
I do quite a bit of web-development and have snippets, automatic indentation 
and autocompletion is a big help . =web-mode.el= is an autonomous emacs 
major-mode for editing web templates. It also takes care of basic stuff like 
syntax highlighting, auto pairing tags, removing white spaces etc. Web-mode is 
feature rich and as extensible as Emacs itself. For more details visit this [[http://web-mode.org/][link]]
#+BEGIN_SRC emacs-lisp
...
Widely used today for almost every sector of computer science from simple 
scripting to ML and AI. I use a few tools to make things easier when I use 
python. Of course none of these are compulsory. Keep what you need and take 
out the rest.
**** Elpy
...
highlighting and code hinting (in the modeline) for python files. Elpy offers 
a lot of features, but the following keybindings should be enough to get started:
...
Please have a look at the documentation at [[https://elpy.readthedocs.io/en/latest/index.html][Readthedocs]] if you want to know 
more about what Elpy can do.
#+BEGIN_SRC emacs-lisp
(use-package elpy
:ensure t
:config
...
#+END_SRC
**** Black 
Black is an opinionated pyton formatter. Install with =pip install black= so 
the command line tool is available.
#+BEGIN_SRC emacs-lisp
  (use-package blacken)
#+END_SRC
**** Kivy
